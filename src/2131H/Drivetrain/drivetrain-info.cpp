...
DrivetrainInfo &DrivetrainInfo::get(float track_width, float track_length,
                                    float robot_mass, float wheel_diameter,
                                    float wheel_max_rpm, uint32_t motor_count,
                                    uint32_t motor_max_rpm) {
  if (track_width <= 0 || track_length <= 0 || robot_mass <= 0 ||
      wheel_diameter <= 0 || wheel_max_rpm <= 0 || motor_count == 0 ||
      motor_max_rpm == 0) {
    throw std::invalid_argument("Invalid parameters for DrivetrainInfo");
  }

  static DrivetrainInfo instance(track_width, track_length, robot_mass,
                                 wheel_diameter, wheel_max_rpm, motor_count,
                                 motor_max_rpm);
  return instance;
}

// Constructor
constexpr DrivetrainInfo::DrivetrainInfo(float track_width, float track_length,
                                         float robot_mass, float wheel_diameter,
                                         float wheel_max_rpm,
                                         uint32_t motor_count,
                                         uint32_t motor_max_rpm)
    : track_width(track_width), track_length(track_length),
      robot_mass(robot_mass), wheel_diameter(wheel_diameter),
      wheel_circumference(wheel_diameter * M_PI), wheel_max_rpm(wheel_max_rpm),
      motor_count(motor_count), motor_max_rpm(motor_max_rpm),
      ...
      max_speed(wheel_circumference * (motor_max_rpm / 60.0) *
                (wheel_max_rpm / motor_max_rpm)),
      max_acceleration((motor_count * motor_max_nm * gear_ratio_to_motor) /
                       ((wheel_diameter / 39.37) * (robot_mass / 2.20462)) *
                       39.37) {}
